...
public class JSONApp {
    ...
    public static JsonObject sessionResult() {
        JsonPair jYear = new JsonPair("year", new JsonNumber(2));
        JsonPair surname = new JsonPair("surname", new JsonString("Rodionov"));
        JsonPair name = new JsonPair("name", new JsonString("Andrii"));
        JsonPair exams = new JsonPair("exams",new JsonArray(
                new JsonObject(new JsonPair("course", new JsonString("OOP")), new JsonPair("mark", new JsonNumber(3)),
                new JsonPair("passed", new JsonBoolean(true))),
                new JsonObject(new JsonPair("course", new JsonString("English")), new JsonPair("mark", new JsonNumber(5)),
                        new JsonPair("passed", new JsonBoolean(true))),
                new JsonObject(new JsonPair("course", new JsonString("Math")), new JsonPair("mark", new JsonNumber(2)),
                        new JsonPair("passed", new JsonBoolean(false)))));
        System.out.println(new JsonObject(name, surname, jYear, exams).toJson());
        return new JsonObject(name, surname, jYear, exams);
    }
}
